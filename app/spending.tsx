...
export default function Spending({ isFocused }: { isFocused: boolean }) {
          ...
          {categoryData.map((category, index) => {
            ...
            return (
              <View key={index} style={{position: 'absolute', transform: [{rotate: rotate + 'deg'}, {scaleX: -1}, {scaleY: -1}]}}>
                <AnimatedCircularProgress size={verticalScale(255.5)} width={verticalScale(8)} fill={fill} tintColor={category.color} lineCap={'round'}/>
              </View>
            )
          })}
        </View>
